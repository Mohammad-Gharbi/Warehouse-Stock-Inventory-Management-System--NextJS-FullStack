...
function getBrevoConfig() {
  ...
  const senderName = getEnvVar("BREVO_SENDER_NAME") || process.env.BREVO_SENDER_NAME || "Techmaster Store";
...
export async function sendEmailViaBrevo(request: SendEmailRequest): Promise<EmailSendResult> {
  try {
    ...
    const emailData = {
      ...
      headers: {
        "X-Mailer": "Techmaster Store Email System",
        "X-Priority": "3",
        Importance: "normal",
        Precedence: "bulk",
 ...
 *
 * @returns string - Sender name (defaults to "Techmaster Store")
 */
